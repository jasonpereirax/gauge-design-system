        ...
        <div style={{ display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:"16px",paddingBottom:"18px" }}>
          <div>
            <div style={{ display:"flex",alignItems:"center",gap:"10px",marginBottom:"6px",flexWrap:"wrap" }}>
              ...
              {version && <span style={{ fontSize:"11px",background:"#eff6ff",color:"#1e40af",padding:"2px 8px",borderRadius:"999px",fontFamily:"monospace",fontWeight:600 }}>v{version}</span>}
              ...
              {lastUpdated && <span style={{ fontSize:"11px",color:"#a1a1aa",marginLeft:"auto" }}>Atualizado {new Date(lastUpdated).toLocaleDateString("pt-BR")}</span>}
            </div>
...
export default function App() {
  ...
  useEffect(()=>{
    fetch("/components.json").then(r=>r.ok?r.json():null).then(data=>{
      if(!data||!data.components)return
      const map:Record<string,any> = {}
      data.components.forEach((c:any)=>{ map[c.safeName||c.name.replace(/[^a-zA-Z0-9]/g,"")] = { version: c.version, lastUpdated: c.lastUpdated } })
      setComponentMeta(map)
    }).catch(()=>{})
  },[])
    ...
    <div className={darkMode?"dark":""}>
      ...
      {activeSection === "Control" && (
        <ComponentDoc
          ...
          status="stable"
          category="Control"
          version={componentMeta["Control"]?.version}
          lastUpdated={componentMeta["Control"]?.lastUpdated}
          width={138}
          height={141}
